      ...
      <section style={sectionStyle}>
        ...
        <Descriptions bordered column={2}>
          <Descriptions.Item label="Nome" span={2}>
            <strong>{capitalizeWords(cliente.name)}</strong>
          </Descriptions.Item>
          {!isCNPJ && (
            <Descriptions.Item label="CPF">
              {formatDocument(cliente.documentNumber, "CPF") || "Não informado"}
            </Descriptions.Item>
          )}
          {isCNPJ && (
            <>
              <Descriptions.Item label="Razão Social">
                {capitalizeWords(cliente.legalName || cliente.name)}
              </Descriptions.Item>
              <Descriptions.Item label="CNPJ">
                {formatDocument(cliente.documentNumber, "CNPJ") || "Não informado"}
              </Descriptions.Item>
            </>
          )}
        ...
        <section style={sectionStyle}>
          ...
          <Descriptions bordered column={2}>
            ...
            <Descriptions.Item label="Endereço" span={2}>
              {[
                capitalizeWords(cliente.address.street),
                cliente.address.number,
                capitalizeWords(cliente.address.complement),
                capitalizeWords(cliente.address.neighborhood),
              ]
                .filter(Boolean)
                .join(", ") || "Não informado"}
            </Descriptions.Item>
            <Descriptions.Item label="Cidade">
              {capitalizeWords(cliente.address.city) || "Não informado"}
            </Descriptions.Item>
            <Descriptions.Item label="Estado">
              {capitalizeWords(cliente.address.state) || "Não informado"}
            </Descriptions.Item>
          </Descriptions>
        </section>
      )}
